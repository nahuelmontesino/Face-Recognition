...
def save_imagenes_from_csv(csv, is_consulta=False):
    ...
    #print(df['Vectores'])
    #print(type(vectores[0]))
    #my_images = literal_eval(vectores[0])
    #print(type(my_images))
    #tuplee = nombres[0],my_images
   # print(type(my_images))
    lengh = len(nombres)
    #print(lengh)
    for x in range(lengh):
        try:
           tuplaImage = nombres[x], literal_eval(vectores[x])
        #print(type(tuplaImage))
        #print(type(vectores[x]))
        #print(vectores[x])
       # print(tuplaImage)
           database.insert_new_image(tuplaImage, is_consulta)
        except:
            continue
