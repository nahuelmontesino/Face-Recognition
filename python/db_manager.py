...
class DatabaseConnection:
    ...
    def insert_new_imagenes(self,images):
        insert_command = "INSERT INTO imagen(nombre,vector,path) VALUES(%s,%s,%s)"
        self.cursor.executemany(insert_command, images)  #execute many para insertar por lote, images es un arrary de imagenes
    

    def query_all_categorias(self):
        self.cursor.execute("SELECT * FROM categoria;")
        categorias = self.cursor.fetchall()  #fetchall convierte en una lista lo que viene 
        for categoria in categorias:
            pprint("cada categoria: {0}".format(categoria))
...
if __name__=='__main__':
    ...
    #databaseConnection.query_all_categorias()
